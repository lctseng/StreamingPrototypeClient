...
int decoder_init() {
    ...
    decoder_data.last_user_data = NULL;
...
int decoder_parse(uint8_t *in_buffer, int cur_size, void *user_data) {
    ...
    int ret, got_picture;
    int first_pass = 1;
    decoder_data.last_user_data = user_data;
    ...
    while (cur_size > 0 || first_pass) {
        ...
        if (decoder_data.packet.size > 0) {
            // a packet is ready!
            ret = avcodec_decode_video2(decoder_data.pCodecCtx, decoder_data.pFrame, &got_picture,
                                        &decoder_data.packet);
            if (ret < 0) {
            ...
            if (got_picture) {
                ...
                sws_scale(decoder_data.img_convert_ctx,
                          (const uint8_t *const *) decoder_data.pFrame->data,
                ...
                if (decoder_data.frame_handler) {
                    decoder_data.frame_handler(decoder_data.pFrameOutput->data[0],
                                               decoder_data.output_size, user_data);
                }
            }
        }
    }
...
int decoder_flush() {
    ...
    while (1) {
        ...
        if (got_picture) {
            ...
            if (decoder_data.frame_handler) {
                decoder_data.frame_handler(decoder_data.pFrameOutput->data[0],
                                           decoder_data.output_size, decoder_data.last_user_data);
            }
        }
    }
    return frame_formed;
